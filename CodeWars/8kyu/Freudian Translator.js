const toFreud=str=>(Array(str.split(" ").length+1).join("sex ").trim())

//other cool solutions
var toFreud=s=>s.replace(/[^ ]+/g,'sex')

const toFreud = string => string.split(" ").map(x => "sex").join(" ");
...
In this kata, the toFreud() function will take a string as its argument, and return a string with every word replaced by the explanation
to everything, according to Freud.*/
