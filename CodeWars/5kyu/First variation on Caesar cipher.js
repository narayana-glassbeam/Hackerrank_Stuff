const movingShift=(s, shift)=> {
    ...
    for (let c of s) {
      if('a'<= c && c <= 'z') {
        res+= String.fromCharCode(c.charCodeAt(0) + shift%26) > 'z'?
          String.fromCharCode(96+(c.charCodeAt(0) + shift%26)%'z'.charCodeAt(0))
          : String.fromCharCode(c.charCodeAt(0) + shift%26)
      } else if ('A'<= c && c <= 'Z') {
        res+= String.fromCharCode(c.charCodeAt(0) + shift%26) >'Z'?
            String.fromCharCode(64+(c.charCodeAt(0) + shift%26)%'Z'.charCodeAt(0))
            : String.fromCharCode(c.charCodeAt(0) + shift%26)
      } else res+=c;
      shift+=1;
    }

  return  divideResponsibility(res,5);
}

const demovingShift=(arr, shift)=>  {
    let res = '';
    for (let c of arr.join('')) {
      if('a'<= c && c <= 'z') {
        res+= String.fromCharCode(c.charCodeAt(0) - shift%26) < 'a'?
          String.fromCharCode(26+(c.charCodeAt(0) - shift%26)%'a'.charCodeAt(0))
          : String.fromCharCode(c.charCodeAt(0) - shift%26)
      } else if ('A'<= c && c <= 'Z') {
        res+= String.fromCharCode(c.charCodeAt(0) - shift%26) <'A'?
            String.fromCharCode(26+(c.charCodeAt(0) - shift%26)%'A'.charCodeAt(0))
...
const divideResponsibility=(s,n)=> {
  ...
  let final = s.match(regex);
  if(!final[4]) final.push("")
  return final;
}
