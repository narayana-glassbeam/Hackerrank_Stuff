const encodeStr=(s, shift) =>{
  ...
  let second = String.fromCharCode(first + shift) > 'z'? String.fromCharCode(96+(first.charCodeAt(0) + shift)%'z'.charCodeAt(0))
    : String.fromCharCode(first.charCodeAt(0) + shift)
  let prefix = first+second;

  let sol =s.split('').map(c=> {
      if('a'<= c && c <= 'z')  return String.fromCharCode(c.charCodeAt(0) + shift) > 'z'?
        String.fromCharCode(96+(c.charCodeAt(0) + shift)%'z'.charCodeAt(0))
        : String.fromCharCode(c.charCodeAt(0) + shift)
      else if ('A'<= c && c <= 'Z') return String.fromCharCode(c.charCodeAt(0) + shift) >'Z'?
          String.fromCharCode(64+(c.charCodeAt(0) + shift)%'Z'.charCodeAt(0))
          : String.fromCharCode(c.charCodeAt(0) + shift)
      else return c
    }).join('')

    let result = prefix+sol;
    return divideResponsibility(result, 5)
}

const divideResponsibility=(s,n)=> {
  let r = Math.ceil(s.length/n);
  let p = `.{1,${r}}`;
...
const decode=arr=> {
  let originalLetter = arr[0][0].charCodeAt(0);
  let encodedLetter = arr[0][1].charCodeAt(0);
  let shift = encodedLetter-originalLetter;
  let res = '';

  res = arr.join('').split('').map(c=> {
    if('a'<= c && c <= 'z')  return String.fromCharCode(c.charCodeAt(0) - shift) < 'a'?
      String.fromCharCode(26+(c.charCodeAt(0) - shift)%'a'.charCodeAt(0))
      : String.fromCharCode(c.charCodeAt(0) - shift)
    else if ('A'<= c && c <= 'Z') return String.fromCharCode(c.charCodeAt(0) - shift) < 'A'?
        String.fromCharCode(26+(c.charCodeAt(0) - shift)%'A'.charCodeAt(0))
        : String.fromCharCode(c.charCodeAt(0) - shift)
    else return c
  }).join('')
