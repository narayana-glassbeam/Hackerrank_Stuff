const leagueCalculate=(team1, team2, result)=> {
  for (let team of leagueTable) {
    if(result==='win' && team1===team[0]) team[1]=team[1]+3;
    if(result==='draw' && team1===team[0]) team[1]=team[1]+1;
    if(result==='draw' && team2===team[0]) team[1]=team[1]+1;
  }
  return leagueTable.sort().sort((a,b)=>b[1]-a[1])
  //it would have been better like this: return leagueTable.sort((a,b)=>(a[1]==b[1])?b[0]<a[0]:b[1]-a[1]);
}

//other solutions
const leagueCalculate=(team1, team2, result)=>{
  leagueTable=leagueTable.map(a=>result=="win" ? (a[0]==team1 ? [a[0],a[1]+3] : a) : (a[0]==team1 || a[0]==team2 ? [a[0],a[1]+1] : a))
    .sort((a,b)=>b[1]-a[1]==0 ? b[0]<a[0] : b[1]-a[1]); return leagueTable;
}
