const isValidHKPhoneNumber=n=>(/^\d{4} \d{4}/.test(n))
const hasValidHKPhoneNumber=s=>(s.search(/\d{4} \d{4}/)>-1)

//other solutions
var isValidHKPhoneNumber=s=>/^\d\d\d\d\s\d\d\d\d$/.test(s);
var hasValidHKPhoneNumber=s=>/\d\d\d\d\s\d\d\d\d/.test(s);

const isValidHKPhoneNumber = number => /^[0-9]{4} [0-9]{4}$/g.test(number);
const hasValidHKPhoneNumber = number => /[0-9]{4} [0-9]{4}/g.test(number);
