const checkDNA=(s1, s2)=>(s1.length>s2.length? solve(s1,s2) : solve(s2,s1))
const solve=(s1,s2)=>(s1.indexOf(convertDNA(s2))>-1? true : false)
const convertDNA=s=>(s.split('').map(c=>c=='A'? 'T' : c=='T'? 'A' : c=='C'? 'G' : 'C').reverse().join(''))

//all in one (similar but not the same)
