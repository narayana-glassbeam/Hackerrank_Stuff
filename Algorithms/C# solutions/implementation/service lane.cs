...
class Solution {
    ...
    static void Main(String[] args) {
        ...
        int[] width = Array.ConvertAll(Console.ReadLine().Split(' '), Int32.Parse);
         ...
         while (N > 0)
            {
                int[] lane = Array.ConvertAll(Console.ReadLine().Split(' '), Int32.Parse);
                int entry = lane[0];
                int exit = lane[1]-lane[0]+1;
                int[] newLane =  width.Skip(entry).Take(exit).ToArray();
                if (newLane.Contains(1)) Console.WriteLine(1);
                else
                {
                    if (newLane.Contains(2)) Console.WriteLine(2);
