object Solution {
    def main(args: Array[String]) {
        val N = readLine.split(' ').toList.last.toInt
        val width = readLine.split(' ').toList.map(_.toInt)
        for (i <- 0 until N) {
            val lane = readLine.split(' ').toList.map(_.toInt)
            val entry = lane(0)
            val exit = lane(1)+1
            val newLane = width.slice(entry, exit)
            if(newLane.contains(1)) println(1)
            else {
                if(newLane.contains(2)) println(2) else println(3)
            }
        }
    }
}
...
//these are slower solutions, because we have to traverse whole array which is unnecessary
println(width.slice(entry, exit).sorted.head)
println(width.slice(entry, exit).min)
